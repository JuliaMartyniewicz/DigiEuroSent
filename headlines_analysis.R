...
# Clean and prepare the 'header_en' column
data <- data %>%
  mutate(header_en = tolower(header_en)) %>%  # Convert to lowercase
  mutate(header_en = gsub("[[:punct:]]", "", header_en)) %>% #Remove punctuation
  mutate(header_en = gsub("[[:digit:]]", "", header_en)) #Remove digits
...
syuzhet_sentiment <- get_sentiment(data$header_en, method = "syuzhet")
data$syuzhet_score <- syuzhet_sentiment

# --- Sentiment Analysis using sentimentr ---
sentences <- get_sentences(data$header_en) # Sentence boundary disambiguation
sentimentr_sentiment <- sentiment(sentences)
data$sentimentr_score <- sentimentr_sentiment$sentiment
...
# Filter out JP, RU, RS countries (because of low number of observations)
data_filtered <- data %>%
...
# Plot (final plot for the thsesis)
...
# Final plots used for the thesis
...
# --- Top Positive, Negative, and Neutral Sources (using sentimentr) ---
...
# Visualize top sources and their sentiment
...
# Left-wing, right-wing and tabloid comparison

# Define left/right/tabloid sources
left_wing <- c(
...
ideology_sentiment <- sentiment_summary %>%
  group_by(ideology) %>%
  slice_max(avg_sentiment, n = 3) %>%   # Top 3 positive for each ideology
  bind_rows(
    sentiment_summary %>%
      group_by(ideology) %>%
      slice_min(avg_sentiment, n = 3)    # Top 3 negative for each ideology
  ) %>%
...
# Filter data and calculate avg sentiment (now for all 3 categories)
sentiment_data <- data %>%
  filter(sitename %in% c(left_wing, right_wing, tabloid_sources)) %>%
  mutate(ideology = case_when(
    sitename %in% left_wing ~ "Left",
    sitename %in% right_wing ~ "Right",
    sitename %in% tabloid_sources ~ "Tabloid",
    TRUE ~ NA_character_ # Fallback, should not be hit if filtering is correct
  )) %>%
  group_by(sitename, ideology) %>%
  summarize(
    avg_sentiment = mean(sentimentr_score, na.rm = TRUE),
    n_articles = n() # Keep n_articles for volume
  ) %>%
  ungroup() %>%
  arrange(ideology, desc(avg_sentiment)) # Sort by ideology and sentiment

# Plot: Sentiment distribution by ideology (3 categories) with volume
ggplot(sentiment_data, aes(
  x = reorder(sitename, avg_sentiment), # Reorder sitename within each facet
  y = avg_sentiment,
  fill = ideology
)) +
  geom_bar(stat = "identity", width = 0.7) +
  geom_text(
    aes(label = paste0(round(avg_sentiment, 2), " (n=", n_articles, ")")), # Include avg_sentiment and n_articles
    hjust = ifelse(sentiment_data$avg_sentiment >= 0, -0.1, 1.1), # Adjust position based on sentiment value
    size = 3
  ) +
  scale_fill_manual(values = c("Left" = "#e74c3c", "Right" = "#3498db", "Tabloid" = "#2ecc71")) + # Colors for 3 categories
  expand_limits(y = c(-0.35, 0.2)) +  # Adjust as needed
  coord_flip() + # Horizontal bars
  facet_wrap(~ideology, scales = "free_y", ncol = 1) + # Separate panels for each ideology, 1 column for vertical stacking
  labs(
    x = "", # Remove x-axis label as sitename is self-explanatory
    y = "Sentiment Score (Higher = More Positive)",
    fill = "Ideology/Media Type" # Updated legend title
  ) +
  theme_minimal() +
  theme(
...
# Identify country codes in the "Other/Unknown" category (just in case)
...
# Sentiment by economic group (using sentimentr)
...
ggplot(group_sentiment_sentimentr, aes(x = economic_group, y = avg_sentiment)) +
  ...
  labs(x = "Economic Group", y = "Average Sentiment") +
  theme_minimal()
...
# Sentiment analysis for specific keywords
...
keywords <- c(
  ...
  "interest rates", "payment system", "cash", "inflation", "money", "payments",
  
  # Cryptocurrencies
  "cryptocurrency", "cryptocurrencies", "crypto", "Bitcoin", "Ethereum", "stablecoin",
  
  # Public Perception
  "privacy", "security", "cashless"
)
...
# Save as an image (used for thesis)
gtsave(keyword_table, "keyword_sentiment_analysis_table.png")
...
ggplot(media_sentiment, aes(x = media_type, y = avg_sentiment)) +
  geom_bar(stat = "identity", fill = "#6699CC") +
  labs(x = "Media Type", y = "Average Sentiment") +
  theme_minimal()
...
emotion_scores <- get_nrc_sentiment(data$header_en)
